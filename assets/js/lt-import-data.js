/*!
 * @package   yii2-import
 * @author    Kartik Visweswaran <kartikv2@gmail.com>
 * @copyright Copyright &copy; Kartik Visweswaran, Krajee.com, 2015 - 2016
 * @version   1.2.5
 *
 * Import Data Validation Module.
 *
 * Author: Kartik Visweswaran
 * Copyright: 2015, Kartik Visweswaran, Krajee.com
 * For more JQuery plugins visit http://plugins.krajee.com
 * For more Yii related demos visit http://demos.krajee.com
 */
(function ($) {
    "use strict";

    var isEmpty = function (value, trim) {
            return value === null || value === undefined || value === [] || value === '' || trim && $.trim(value) === '';
        },
        popupDialog = function (url, name, w, h) {
            var left = (screen.width / 2) - (w / 2), top = 60,
                existWin = window.open('', name, '', true);
            existWin.close();
            return window.open(url, name,
                'toolbar=no, location=no, directories=no, status=yes, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=' + w + ', height=' + h + ', top=' + top + ', left=' + left);
        },
        popupTemplate = '<html style="display:table;width:100%;height:100%;">' +
            '<title>Import Data - &copy; Krajee</title>' +
            '<body style="display:table-cell;font-family:Helvetica,Arial,sans-serif;color:#888;font-weight:bold;line-height:1.4em;text-align:center;vertical-align:middle;width:100%;height:100%;padding:0 10px;">' +
            '{msg}' +
            '</body>' +
            '</html>',
        ltConfirm = function (msg) {
            if (isEmpty(msg)) {
                return true;
            }
            try {
                return window.confirm(msg);
            }
            catch (err) {
                return true;
            }
        };

    var ImportData = function (element, options) {
        var self = this;
        self.$element = $(element);
        $.each(options, function (key, val) {
            self[key] = val;
        });
        var settings = options.settings;
        self.$form = $("#" + settings.formId);
        self.messages = settings.messages;
        self.popup = '';
        self.listen();
    };

    ImportData.prototype = {
        constructor: ImportData,
        notify: function (e) {
            var self = this, msgs, msg = '', msg1, msg2, msg3;
            if (!self.showConfirmAlert) {
                e.preventDefault();
                return true;
            }
            msgs = self.messages;
            msg1 = isEmpty(self.alertMsg) ? '' : self.alertMsg;
            msg2 = isEmpty(msgs.allowPopups) ? '' : msgs.allowPopups;
            msg3 = isEmpty(msgs.confirmDownload) ? '' : msgs.confirmDownload;
            if (msg1.length && msg2.length) {
                msg = msg1 + '\n\n' + msg2;
            } else {
                if (!msg1.length && msg2.length) {
                    msg = msg2;
                } else {
                    msg = (msg1.length && !msg2.length) ? msg1 : '';
                }
            }
            if (msg3.length) {
                msg = msg + '\n\n' + msg3;
            }
            e.preventDefault();
            if (isEmpty(msg)) {
                return true;
            }
            return ltConfirm(msg);
        },
        setPopupAlert: function (msg) {
            var self = this;
            if (!self.popup.document) {
                return;
            }
            if (arguments.length && arguments[1]) {
                var el = self.popup.document.getElementsByTagName('body');
                setTimeout(function () {
                    el[0].innerHTML = msg;
                }, 4000);
            } else {
                var newmsg = popupTemplate.replace('{msg}', msg);
                self.popup.document.write(newmsg);
            }
        },
        listen: function () {
            var self = this;
            self.$form.attr('action', window.location.href).appendTo('body');
            self.$element.off('click').on('click', function (e) {
                var $selected, cols = [];
                if (self.notify(e)) {
                    var fmt = $(this).data('format');
                    self.$form.find('[name="import_type"]').val(fmt);
                    if (self.target === '_popup') {
                        self.popup = popupDialog('', 'ltImportDialog', 350, 120);
                        self.popup.focus();
                        self.setPopupAlert(self.messages.downloadProgress);
                    }
                    if (!isEmpty(self.columnSelectorId)) {
                        $selected = $('#' + self.columnSelectorId).parent().find('input[name="import_columns_selector[]"]');
                        $selected.each(function () {
                            var $el = $(this);
                            if ($el.is(':checked')) {
                                cols.push($el.attr('data-key'));
                            }
                        });
                        self.$form.find('input[name="import_columns"]').val(cols.join(','));
                    }
                    self.$form.trigger('submit');
                }
            });
            if (self.target === '_popup') {
                self.$form.off('submit').on('submit', function () {
                    setTimeout(function () {
                        self.setPopupAlert(self.messages.downloadComplete, true);
                    }, 1000);
                });
            }
        }
    };

    //ImportData plugin definition
    $.fn.importdata = function (option) {
        var args = Array.apply(null, arguments);
        args.shift();
        return this.each(function () {
            var $this = $(this),
                data = $this.data('importdata'),
                options = typeof option === 'object' && option;

            if (!data) {
                $this.data('importdata',
                    (data = new ImportData(this, $.extend({}, $.fn.importdata.defaults, options, $(this).data()))));
            }

            if (typeof option === 'string') {
                data[option].apply(data, args);
            }
        });
    };

    $.fn.importdata.defaults = {
        filename: 'import',
        target: '_popup',
        showConfirmAlert: true,
        columnSelectorId: null,
        alertMsg: '',
        settings: {
            formId: '',
            messages: {
                allowPopups: '',
                confirmDownload: '',
                downloadProgress: '',
                downloadComplete: ''
            }
        }
    };
})(window.jQuery);